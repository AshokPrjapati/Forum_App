...
export const getUser =
  (id: string, Toast: Function) => async (dispatch: Dispatch) => {
    if (!id) return;
    ...
    try {
      ...
      sessionStorage.setItem(
        "user",
        JSON.stringify({
          id: response.data.credentials._id,
...
export const GoogleAuth =
  (navigate: Function, Toast: Function) => async (dispatch: Dispatch<any>) => {
    dispatch({ type: Types.AUTH_LOADING });
    try {
...
export const login =
  (email: string, password: string, navigate: Function, Toast: Function) =>
...
export const signup =
  (userData: UserI, navigate: Function, Toast: Function) =>
  async (dispatch: Dispatch) => {
    dispatch({ type: Types.AUTH_LOADING });
    try {
...
export const logout =
  (email: string, Toast: Function, navigate: Function) =>
  async (dispatch: Dispatch) => {
    dispatch({ type: Types.AUTH_LOADING });
    try {
...
export const sendVerifyEmail =
  (email: string, password: string, Toast: Function) =>
  async (dispatch: Dispatch) => {
    dispatch({ type: Types.AUTH_LOADING });
    try {
      let res = await axios.post("/user/sentverificationemail", {
        email,
        password,
      });
      console.log("res: ", res.data.EncryptedCredential);
      dispatch({
        type: Types.SEND_VERIFY_EMAIL_SUCCESS,
        payload: res.data.EncryptedCredential,
      });
      Toast(
...
export const updateUser =
  (userData: IUser, onClose: Function, Toast: Function) =>
  async (dispatch: Dispatch) => {
    dispatch({ type: Types.AUTH_LOADING });
...
export const followUser =
  (
    data: { userID: string; followingID: string },
    Toast: Function,
    fetchUser?: Function
  ) =>
...
export const getFollower =
  (userID: string, Toast: Function) => async (dispatch: Dispatch<any>) => {
    try {
      const response = await axios.get(`/user/${userID}/follower`);
      dispatch({
        type: Types.GET_USER_FOLLOWER,
