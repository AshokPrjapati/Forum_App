...
export const getAllPost = () => async (dispatch: Dispatch) => {
  ...
  try {
    ...
    const FinalPosts = posts.map((post: IPost) => {
      const { null: RootComments, ...Replies } = commentsWithParentId(
        post.comments
      );
      if (RootComments) {
        RootComments.sort(
...
export const getSingleUserAllPost =
  (_id: string) => async (dispatch: Dispatch) => {
    ...
    try {
      ...
      dispatch({
        type: Types.GET_SINGLE_USER_ALLPOST_SUCCESS,
        payload: FinalPosts,
      });
    } catch (error) {
...
export const updatePost =
  (id: string, data: { title: string; description: string; image?: any }) =>
...
export const likePost =
  (id: string, userId: string) => async (dispatch: Dispatch<any>) => {
    dispatch({ type: Types.POST_LOADING });
    try {
