    ...
    if (decode) {
      ...
      if (user) {
        if (user.password == decode.password) {
          ...
          return res
            .status(201)
            .json({ status: 403, message: "Wrong credential" });
        }
      } else {
        return res
          .status(201)
...
async function UserFollower(req, res) {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ status: 200, message: "user follower.", followers });
  } catch (error) {
    console.log("error: ", error);
    return res.status(201).json({ status: 401, error: error.message });
  }
...
async function UserAllFollow(req, res) {
  const id = req.params.id;
  try {
    const following = await FollowModel.find({ userID: id }).populate(
      "followingID"
    );
    ...
    return res
      .status(201)
      .json({ status: 200, message: "user following.", following, followers });
  } catch (error) {
    console.log("error: ", error);
    return res.status(201).json({ status: 401, error: error.message });
  }
